...
class VertxHttpClientTest extends AbstractHttpClientTest<Future<HttpClientRequest>> {
  ...
  public void sendRequestWithCallback(
    ...
    sendRequest(request)
        .whenComplete((status, throwable) -> requestResult.complete(() -> status, throwable));
  }
    ...
    optionsBuilder.setHttpAttributes(
        uri -> {
          switch (uri.toString()) {
            ...
            default:
              return new HashSet<>(HttpClientTestOptions.DEFAULT_HTTP_ATTRIBUTES);
          }
        });
  ...
  private static SingleConnection createSingleConnection(String host, int port) {
    return new VertxSingleConnection(host, port);
  }
}
